...
module.exports = {
    ...
    async execute(message, args, client, prefix) {
        ...
        const options = [
            'verified', 'vip', 'vip+', 'mvp', 'mvp+', 'mvp++', 'youtuber', 'game master', 'admin'
        ], full = [
            'verified', 'VIP', 'VIP_PLUS', 'MVP', 'MVP_PLUS', 'SUPERSTAR', 'YOUTUBER', 'GAME_MASTER', 'ADMIN'
        ]
        ...
        if (args.length === 0) {
            const embed = new Discord.MessageEmbed()
                .setColor(e.green)
                .setDescription(
                    `:flushed: __**Set Role as Rank**__\n` +
                    `Role type list: \`Verified, VIP, VIP+, MVP, MVP+, MVP++,\`\n` +
                    `\`YouTuber, Game Master, Admin\`\n\n` +
                    `**Command example**:\n` +
                    `\`${prefix}setrole [roletype] [@rolemention or role id]\`\n` +
                    `\`${prefix}setrole VIP 755997125099388979\``
                )
                ...
                if (message.guild.roles.cache.get(roleid) == null || message.guild.roles.cache.get(roleid) === undefined) {
                    return message.channel.send(
                        new Discord.MessageEmbed()
                            .setColor(e.red)
                            .setDescription(`${e.x} **Invalid role**\n\n\`${prefix}setrole [roletype] [@mention or role id]\`n\`${prefix}setrole VIP 755997125099388979\``)
                    );
                }
