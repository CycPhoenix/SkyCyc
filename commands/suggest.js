        ...
        if (blacklist.indexOf(message.author.id) > -1) {
            ...
            return message.channel.send(
                new Discord.MessageEmbed()
                    ...
                    .setDescription(
                        `You're blacklisted from using the suggest command for SkyCyc.\n\n` +
            ...
            suggestChannel.send(`${message.author.tag} | ${message.author.id}\n${args.join(' ').substr(0, 1000)}`).then(() => {
                const embed = new Discord.MessageEmbed()
                    ...
                    .setDescription(
                        ...
                        `**:point_right: PLEASE NOTE THAT THIS COMMAND IS FOR ONLY [SKYCYC] :point_left:\n` +
                        `**If your idea is added, Cyc will DM you 😳`);
                message.channel.send(embed).then((newmsg) => {
